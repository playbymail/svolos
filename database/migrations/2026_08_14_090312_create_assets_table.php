...
return new class extends Migration
{
     ...
     * Run the migrations.
     *
     * What an entity owns: a quantity of one kind of asset, in one assignment.
     *
     ...
     * than a thousand. `(entity_id, type, assignment)` is unique so that there is exactly one answer
     * to "how much of this does it have, here" — two rows for the same kind in the same assignment
     ...
     *
     * ## `assignment` is a column because somebody decided it
     *
     ...
     * Which assignments a kind may legally sit in is a rule on `App\Enums\AssetType`, enforced by
     * `App\Generation\AssetHolding`'s constructor rather than by a check constraint.
     *
     * ## The cascade has to be the database's, not the model's
     *
     * `GenerateAssets::discard()` mass-deletes entities, and a mass delete fires no model events — so
     * an `deleting` hook here would never run and the assets would be orphaned. Cascading in the
